...
Write-Host " IPPLIAP - PS34 EXCEL ANALITICO + INTEGRANTES" -ForegroundColor Green
Write-Host "========================================================" -ForegroundColor DarkGreen
...
if ($Branch -ne "main") {
    throw "PS34 debe ejecutarse en la rama main."
}

$FormService = ".\app\services\institutional_form_service.py"
$ExportService = ".\app\services\institutional_export_service.py"

if (-not (Test-Path $FormService)) { throw "No existe $FormService" }
if (-not (Test-Path $ExportService)) { throw "No existe $ExportService" }

$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$BackupDir = Join-Path $env:TEMP "IPPLIAP_PS34_BACKUP_$Stamp"
New-Item -ItemType Directory -Path $BackupDir -Force | Out-Null

Copy-Item $FormService (Join-Path $BackupDir "institutional_form_service.py") -Force
Copy-Item $ExportService (Join-Path $BackupDir "institutional_export_service.py") -Force

Write-Host "Backup fuera del repo:" $BackupDir -ForegroundColor Yellow

$PatchPy = Join-Path $env:TEMP "ippliap_ps34_patch_$Stamp.py"

$PatchCode = @'
from pathlib import Path

form_path = Path(r"app/services/institutional_form_service.py")
text = form_path.read_text(encoding="utf-8")

start_marker = "def calculate_reference(data,family_members=None,income_rows=None):"
end_marker = "def full_address(data):"

start = text.find(start_marker)
end = text.find(end_marker)

if start < 0:
    raise SystemExit("No se encontro calculate_reference.")
if end < 0 or end <= start:
    raise SystemExit("No se encontro full_address despues de calculate_reference.")

new_block = r"""__CALC_BLOCK__"""

updated = text[:start] + new_block + text[end:]
form_path.write_text(updated, encoding="utf-8", newline="\n")
print("FORM_SERVICE_OK")
'@

$PatchCode = $PatchCode.Replace("__CALC_BLOCK__", @'
...
python $PatchPy
if ($LASTEXITCODE -ne 0) { throw "Fallo al corregir el calculo de integrantes." }
...
Set-Content -LiteralPath $ExportService -Value $ExportCode -Encoding UTF8

Write-Host ""
Write-Host "[1] Compilacion" -ForegroundColor Cyan
python -m py_compile $FormService $ExportService
if ($LASTEXITCODE -ne 0) { throw "Fallo de compilacion." }
Write-Host "OK" -ForegroundColor Green

Write-Host ""
Write-Host "[2] Prueba puntual de integrantes" -ForegroundColor Cyan
python -c "from app.services.institutional_form_service import calculate_reference; d={'household_size':'2','renta':'4500','luz':'600','agua':'350','telefono_gasto':'450','gas':'650','alimentos':'4000','pasajes':'1200','vestido':'600','medico_medicinas':'500','creditos_personales':'700','otros_gastos':'300'}; f=[{'nombre':'Maria'},{'nombre':'Mateo'},{'nombre':'Carlos'}]; r=[{'aportacion_mensual':'10500'},{'aportacion_mensual':'5000'}]; c=calculate_reference(d,f,r); print(c); assert c['household_size']==3; assert c['income_per_capita']==5166.67; assert c['available_per_capita']==550.0"
if ($LASTEXITCODE -ne 0) { throw "Fallo la prueba de integrantes." }
Write-Host "OK" -ForegroundColor Green

Write-Host ""
Write-Host "[3] Cambios Git" -ForegroundColor Cyan
git status --short
git diff -- app/services/institutional_form_service.py app/services/institutional_export_service.py

Write-Host ""
Write-Host "========================================================" -ForegroundColor DarkGreen
Write-Host " PS34 COMPLETADO" -ForegroundColor Green
Write-Host "========================================================" -ForegroundColor DarkGreen
Write-Host "Ahora ejecute: .\35_validar_excel_analitico.ps1"
